...
gen-wire-deps:
	cd cmd/rapid-product-catalog && wire && cd ../..

compile:
	mkdir -p out/
	go build -o $(APP_EXECUTABLE) -ldflags "-X main.version=$(APP_VERSION) -X main.commit=$(APP_COMMIT)" ./cmd/rapid-product-catalog
...
test-cover-html:
	...
	cat cover.out.tmp | grep -v "_mock.go" | grep -v "utils/time.go\|health/service.go" > coverage-all.out
	rm cover.out.tmp
